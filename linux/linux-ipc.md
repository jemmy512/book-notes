...
```c
bool sig_ignored(struct task_struct *t, int sig, bool force)
{
...
bool sig_task_ignored(struct task_struct *t, int sig, bool force)
{
    void __user *handler;
    ...
    init = is_global_init(t) {
        return task_tgid_nr(tsk) == 1
    }
    if (unlikely(init && sig_kernel_only(sig)))
        return true;
...
/* 1. handle CLD signal, set at prepare_signal */
    ...
    for (;;) {
        ...
        if ((signal->flags & SIGNAL_GROUP_EXIT) || signal->group_exec_task) {
            ...
            recalc_sigpending() {
                pending = recalc_sigpending_tsk(current) {
...
/* 2. dequeue pending sig from thread-wide or process-wide signal queue */
...
void do_jobctl_trap(void)
{
    ...
    if (current->ptrace & PT_SEIZED) {
        if (!signal->group_stop_count && !(signal->flags & SIGNAL_STOP_STOPPED))
            signr = SIGTRAP;
        WARN_ON_ONCE(!signr);
        ptrace_do_notify(signr, signr | (PTRACE_EVENT_STOP << 8), CLD_STOPPED, 0);
    } else {
        WARN_ON_ONCE(!signr);
...
void collect_signal(int sig, struct sigpending *list, kernel_siginfo_t *info,
               struct sigqueue **timer_sigq)
{
...
int ptrace_signal(int signr, kernel_siginfo_t *info, enum pid_type type)
{
    ...
    if (sigismember(&current->blocked, signr) || fatal_signal_pending(current)) {
...
```c
int get_signal() {
    /* other tasks in group respond the stop request from initiate task */
    if (unlikely(current->jobctl & JOBCTL_STOP_PENDING) && do_signal_stop(0))
        goto relock;

    /* stop initiate task requests stop for other tasks in group */
    if (sig_kernel_stop(signr))
        ret = do_signal_stop(ksig->info.si_signo);
}
    ...
    __releases(&current->sighand->siglock)
{
    ...
    /* SIGSTIP initiator requests other threads to stop  */
    ...
    /* STOP SIGSTOP initiator/other threads */
...
[:link: rseq_signal_deliver](./linux-proc.md#rseq_signal_deliver)
...
int setup_sigframe(struct rt_sigframe_user_layout *user,
...
```c
int get_sigframe(struct rt_sigframe_user_layout *user,
                 ...
                 bool add_all)
{
    ...
    if (system_supports_fpsimd()) {
        err = sigframe_alloc(user, &user->fpsimd_offset, sizeof(struct fpsimd_context));
        if (err)
            return err;
    }
    ...
    if (add_all || current->thread.fault_code) {
        err = sigframe_alloc(user, &user->esr_offset, sizeof(struct esr_context));
        if (err)
            return err;
    }
    ...
    if (system_supports_gcs() && (add_all || current->thread.gcspr_el0)) {
        err = sigframe_alloc(user, &user->gcs_offset, sizeof(struct gcs_context));
        if (err)
            return err;
    }
#endif
    ...
    if (system_supports_sve() || system_supports_sme()) {
        ...
        err = sigframe_alloc(user, &user->sve_offset, SVE_SIG_CONTEXT_SIZE(vq));
        if (err)
            return err;
    }

    if (system_supports_tpidr2()) {
        err = sigframe_alloc(user, &user->tpidr2_offset, sizeof(struct tpidr2_context));
        if (err)
            return err;
    }
...
int __sigframe_alloc(struct rt_sigframe_user_layout *user,
...
```c
int setup_return(struct pt_regs *regs, struct ksignal *ksig,
             struct rt_sigframe_user_layout *user, int usig)
{
    ...
    if (ksig->ka.sa.sa_flags & SA_RESTORER)
        ...
        sigtramp = VDSO_SYMBOL(current->mm->context.vdso, sigtramp) {
            SYM_CODE_START(__kernel_rt_sigreturn)
                mov    x8, #__NR_rt_sigreturn /* mv rt_sigreturn addr to x8 */
                svc    #0                     /* call rt_sigreturn */
    ...
    regs->regs[30] = (unsigned long)sigtramp; /* link reg */
...
vm_fault_t shmem_fault(struct vm_fault *vmf) {
    ...
    /* find page in [page cache, swap, or allocate] */
    err = shmem_get_folio_gfp(inode, vmf->pgoff, 0, &folio, SGP_CACHE, gfp, vmf, &ret);
    if (err)
...
repeat:
    ...
    *foliop = NULL;
    ...
    if (orders > 0) {
        ...
        folio = shmem_alloc_and_add_folio(vmf, huge_gfp, inode, index, fault_mm, orders);
        if (!IS_ERR(folio)) {
...
```c
struct folio *shmem_alloc_and_add_folio(struct vm_fault *vmf,
        ...
        folio = shmem_alloc_folio(gfp, 0, info, index) {
            ...
            folio = folio_alloc_mpol(gfp, order, mpol, ilx, numa_node_id()) {
                return folio_alloc_mpol_noprof(...) {
                    struct page *page = alloc_pages_mpol(gfp | __GFP_COMP, order, pol, ilx, nid);
                    if (!page)
                        return NULL;
