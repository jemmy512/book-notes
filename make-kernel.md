...
# enable related kernel configs
```

Download any missing *.so dependencies from [archlinuxarm.org](https://archlinuxarm.org/packages/aarch64/perf), mv the *.so to `usr/lib` since perf is dynamic linked.